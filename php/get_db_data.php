...
function createSqlstring($fieldArray, $condArray)
{
    ...
    $sql = $sql." ORDER BY measure_date DESC, measure_time DESC";
 ...
 *  If $recordNum is equal to 0, all the data of DB will be displayed.
 */
function getRecentData($recordNum) 
{
    ...
    $recentRecordSql = "SELECT * FROM record_list ORDER BY measure_date DESC, measure_time DESC";

    if($recordNum !== 0) $recentRecordSql = $recentRecordSql." LIMIT ".$recordNum;
